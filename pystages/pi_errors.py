...
class PIError(int, Enum):
    ...
    PI_CNTR_COMMAND_TOO_LONG = 3 # Command length out of limits or command buffer overrun
    ...
    PI_CNTR_SET_PIVOT_NOT_POSSIBLE = 9 # Attempt to set pivot point while U,V and W not all 0
    PI_CNTR_STOP = 10 # Controller was stopped by command
    PI_CNTR_SST_OR_SCAN_RANGE = 11 # Parameter for SST or for one of the embedded scan algorithms out of range
    ...
    PI_CNTR_INVALID_EGE_AXIS = 41 # Invalid axis for electronic gearing, axis can not be slave
    PI_CNTR_SLAVE_POSITION_OUT_OF_RANGE = 42 # Position of slave axis is out of range
    PI_CNTR_COMMAND_EGE_SLAVE = 43 # Slave axis cannot be commanded directly when electronic gearing is enabled
    ...
    PI_CNTR_OPM_NOT_INITIALIZED = 47 # OPM (Optical Power Meter) not initialized or cannot be initialized
    ...
    PI_CNTR_REF_WITH_REF_DISABLED = 50 # Attempt to reference axis with referencing disabled
    ...
    PI_CNTR_INVALID_RECORDER_SRC_OPT = 58 # Source does not exist; number too low or too high
    PI_CNTR_INVALID_RECORDER_SRC_CHAN = 59 # Source Record Table number too low or too high
    PI_CNTR_PARAM_PROTECTION = 60 # Protected Param: current Command Level (CCL) too low
    PI_CNTR_AUTOZERO_RUNNING = 61 # Command execution not possible while Autozero is running
    ...
    PI_CNTR_NOT_ENOUGH_DDL_MEMORY = 68 # Not enough memory available for DDL table; DDL can not be started
    PI_CNTR_DDL_TIME_DELAY_TOO_LARGE = 69 # Time delay larger than DDL table; DDL can not be started
    PI_CNTR_DIFFERENT_ARRAY_LENGTH = 70 # The requested arrays have different lengths; query them separately
    PI_CNTR_GEN_SINGLE_MODE_RESTART = 71 # Attempt to restart the generator while it is running in single step mode
    ...
    PI_CNTR_WAVE_GENERATOR_ACTIVE = 73 # Motion commands are not allowed when wave generator is active
    PI_CNTR_AUTOZERO_DISABLED = 74 # No sensor channel or no piezo channel connected to selected axis (sensor andpiezo matrix)
    PI_CNTR_NO_WAVE_SELECTED = 75 # Generator started (WGO) without having selected a wave table (WSL).
    PI_CNTR_IF_BUFFER_OVERRUN = 76 # Interface buffer did overrun and command couldn't be received correctly
    PI_CNTR_NOT_ENOUGH_RECORDED_DATA = 77 # Data Record Table does not hold enough recorded data
    PI_CNTR_TABLE_DEACTIVATED = 78 # Data Record Table is not configured for recording
    PI_CNTR_OPENLOOP_VALUE_SET_WHEN_SERVO_ON = 79 # Open-loop commands (SVA,SVR) are not allowed when servo is on
    ...
    PI_CNTR_NOT_ALLOWED_IN_CURRENT_MOTION_MODE = 89 # Command not allowed in current motion mode
    ...
    PI_CNTR_SLAVE_NOT_FAST_ENOUGH = 92 # Stage is not capable of following the master. Check the gear ratio.
    ...
    PI_CNTR_OPEN_LOOP_JOYSTICK_ENABLED = 94 # Servo cannot be switched on when open-loop joystick control is activated.
    PI_CNTR_INVALID_SERVO_STATE_FOR_PARAMETER = 95 # This parameter cannot be changed in current servo mode.
    ...
    PI_CNTR_DTR_HINDERS_VELOCITY_CHANGE = 213 # VEL command not allowed in DTR Command Mode
    PI_CNTR_POSITION_UNKNOWN = 214 # Position calculations failed
    PI_CNTR_CONN_POSSIBLY_BROKEN = 215 # The connection between controller and stage may be broken
    ...
    PI_CNTR_UNEXPECTED_STRUT_STOP = 217 # Strut test command failed because of an unexpected strut stop
    PI_CNTR_POSITION_BASED_ON_ESTIMATION = 218 # While MOV! is running position can only beestimated!
    PI_CNTR_POSITION_BASED_ON_INTERPOLATION = 219 # Position was calculated during MOV motion
    PI_CNTR_INTERPOLATION_FIFO_UNDERRUN = 220 # FIFO buffer underrun during interpolation
    PI_CNTR_INTERPOLATION_FIFO_OVERFLOW = 221 # FIFO buffer overflow during interpolation
    ...
    PI_CNTR_OPEN_LOOP_MOTION_SET_WHEN_SERVO_ON = 303 # Open-loop motion attempted when servo ON
    ...
    PI_CNTR_EMERGENCY_STOP_BUTTON_ACTIVATED = 500 # The \"red knob\" is still set and disables system
    ...
    PI_CNTR_COLLISION_SWITCH_ACTIVATED = 503 # Hardware collision sensor(s) are activated
    ...
    PI_CNTR_LOST_SPI_SLAVE_CONNECTION = 507 # Digital connection to external SPI slave device is lost
    PI_CNTR_MOVE_ATTEMPT_NOT_PERMITTED = 508 # Move attempt not permitted due to customer or limit settings
    ...
    PI_CNTR_PARENT_NODE_DOES_NOT_EXIST = 531 # A command refers to a node that has no parent node
    ...
    PI_CNTR_HEXAPOD_IN_MOTION = 536 # Transformation cannot be defined as long as Hexapod is in motion
    PI_CNTR_TRANSFORMATION_TYPE_NOT_SUPPORTED = 537 # Transformation node cannot be activated
    ...
    PI_CNTR_NODE_MEMORY_FULL = 543 # Unused nodes must be deleted before new nodes can be stored
    PI_CNTR_PIVOT_POINT_FEATURE_NOT_SUPPORTED = 544 # With some transformations pivot point usage is not supported
    PI_CNTR_SOFTLIMITS_INVALID = 545 # Soft limits invalid due to changes in coordinate system 
    ...
    PI_CNTR_CS_DATA_CANNOT_BE_QUERIED = 551 # This query is not supported for this coordinate system type
    PI_CNTR_CS_COMBINATION_DOES_NOT_EXIST = 552 # This combination of work-and-tool coordinate system does not exist
    PI_CNTR_CS_COMBINATION_INVALID = 553 # The combination must consist of one work and one tool coordinate system
    ...
    PI_CNTR_CS_TYPE_NOT_ACTIVATED = 556 # No coordinate system of this type is activated
    PI_CNTR_CS_NAME_INVALID = 557 # Name of coordinate system is invalid
    PI_CNTR_CS_GENERAL_FILE_MISSING = 558 # File with stored CS systems is missing or erroneous
    PI_CNTR_CS_LEVELING_FILE_MISSING = 559 # File with leveling CS is missing or erroneous
    ...
    PI_CNTR_POSITION_ERROR_TOO_HIGH = 604 # Position error of any axis in the system is too high
    ...
    PI_CNTR_FAST_ALIGNMENT_PROCESS_IS_NOT_RUNNING = 608 # Fast alignment process cannot be paused because it is not running
    ...
    PI_CNTR_UNABLE_TO_SET_PARAM_WITH_SPA = 650 # Parameter could not be set with SPA - SEP needed?
    ...
    PI_CNTR_OVER_CURR_PROTEC_TRIGGERED_BY_I2T = 655 # Overcurrent protection triggered by I2T-module
    PI_CNTR_OVER_CURR_PROTEC_TRIGGERED_BY_AMP_MODULE = 656 # Overcurrent protection triggered by amplifier module
    PI_CNTR_SAFETY_STOP_TRIGGERED = 657 # Safety stop triggered
    PI_SENSOR_OFF = 658 # Sensor off?
    PI_CNTR_PARAM_CONFLICT = 659 # Parameter could not be set. Conflict with another parameter.
    PI_CNTR_COMMAND_NOT_ALLOWED_IN_EXTERNAL_MODE = 700 # Command not allowed in external mode
    ...
    PI_CNTR_CONFIGURATION_FAILURE_EXTERNAL_MODE = 718 # Missing or incorrect configuration of external mode
    ...
    PI_CNTR_SURFACEDETECTION_RUNNING = 731 # Command not allowed while surface detection is running
    PI_CNTR_SURFACEDETECTION_FAILED = 732 # Last surface detection failed
    PI_CNTR_FIELDBUS_IS_ACTIVE = 733 # Fieldbus is active and is blocking GCS control commands
    ...
    PI_CNTR_INVALID_IDENTIFIER = 1006 # Invalid identifier (invalid special characters, ...)
    ...
    PI_CNTR_MACRO_NO_ANSWER = 1010 # No response was received while executing WAC/MEX/JRC/...
    ...
    PI_CNTR_NO_MACRO_OR_EMPTY = 1013 # No macro with given name on controller, or macro is empty
    ...
    PI_CNTR_PROFILE_INDEX_OUT_OF_RANGE = 1066 # User Profile Mode: Block or Data Set index out of allowed range 
    PI_CNTR_PROFILE_OUT_OF_MEMORY = 1071 # User Profile Mode: Out of memory
    PI_CNTR_PROFILE_WRONG_CLUSTER = 1072 # User Profile Mode: Cluster is not assigned to this axis
    PI_CNTR_PROFILE_UNKNOWN_CLUSTER_IDENTIFIER = 1073 # Unknown cluster identifier
    PI_CNTR_TOO_MANY_TCP_CONNECTIONS_OPEN = 1090 # There are too many open tcpip connections
    ...
    PI_CNTR_PCC_SCAN_OUT_OF_RANGE = 5003 # Defined scan area is larger than the travel range
    ...
    PI_CNTR_PCC_TABLE_ARRAY_TOO_LARGE = 5006 # PicoCompensation table is larger than maximum table length
    PI_CNTR_NEXLINE_ERROR = 5100 # Common error in NEXLINE® firmware module
    PI_CNTR_CHANNEL_ALREADY_USED = 5101 # Output channel for NEXLINE® can not be redefined for other usage
    PI_CNTR_NEXLINE_TABLE_TOO_SMALL = 5102 # Memory for NEXLINE® signals is too small
    PI_CNTR_RNP_WITH_SERVO_ON = 5103 # RNP can not be executed if axis is in closed loop
    ...
    COM_RS_INVALID_DATA_BITS = -30 # RS-232: 5 data bits with 2 stop bits is an invalid combination, as is 6, 7, or 8 data bits with 1.5 stop bits
    COM_ERROR_RS_SETTINGS = -31 # RS-232: Error configuring the COM port
    COM_INTERNAL_RESOURCES_ERROR = -32 # Error dealing with internal system resources (events, threads, ...)
    COM_DLL_FUNC_ERROR = -33 # A DLL or one of the required functions could not be loaded
    ...
    COM_FTDIUSB_DEVICE_NOT_OPENED_FOR_ERASE = -41 # FTDIUSB: device not opened for erase
    COM_FTDIUSB_DEVICE_NOT_OPENED_FOR_WRITE = -42 # FTDIUSB: device not opened for write 
    ...
    COM_PCI_ACCESS_DENIED = -61 # Driver could not be opened (on Vista: run as administrator!)
    ...
    def __str__(self):
        return {self.PI_CNTR_NO_ERROR: "No error",
            ...
            self.COM_USB_NOT_SUPPORTED: "USB is not supported"}[self]
